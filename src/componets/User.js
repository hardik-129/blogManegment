...
function User() {
  ...
  const showButton = (id) => {
    console.log(id);
    setInput("");
    Http.callApi("get", url + `users/${id}/show`).then((res) => {
  ...
  const updateUser = (event) => {
    ...
    axios
      .post(
        `https://blog-api-dev.octalinfotech.com/api/users/${input.id}/update`,
        data,
        { headers: { Authorization: `Bearer ${token}` } }
      )
  ...
  function User(TitleSearchFilter = "") {
    Http.callApi("get", url + `users?search=${TitleSearchFilter}`)
      .then((response) => {
  ...
  return (
    <>
      <form class="flex items-center absolute left-96 mt-8 ">
      ...
      <div className="flex justify-end m-6">
        <button
          className="text-white text-xl  bg-black -400-700 hover:bg-slate-700  focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded  px-5 py-2.5 text-center me-2 mb-2"
